...
function updateKeybindIndicator() {
  ...
  if (!window.expectedNote || !Array.isArray(window.expectedNote)) {
    indicator.textContent = "";
    return;
  }

  // Find the key mapping for the expected notes
  const keybinds = [];
  window.expectedNote.forEach((note) => {
    const keyObj = keyMap.find((k) => k.note.includes(note));
    if (keyObj && keyObj.key) {
      keybinds.push(keyObj.key.toUpperCase());
    }
  });

  if (keybinds.length > 0) {
    indicator.textContent = keybinds.join(" + ");
  } else {
    indicator.textContent = "";
  }
}
